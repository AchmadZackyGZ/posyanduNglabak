<script lang="ts">
	...
	function formatTanggalJadwal(tgl: string) {
		...
		const dmy = date.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
...
<div class="space-y-6">
	...
	{#if isLoading}
		<div class="flex h-64 w-full items-center justify-center rounded-xl bg-white shadow-sm border border-gray-100">
	...
	{:else}
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
			<div class="flex items-center gap-4 rounded-xl border-t-4 border-t-teal-500 bg-white p-5 shadow-sm">
			...
			<div class="flex items-center gap-4 rounded-xl border-t-4 border-t-orange-400 bg-white p-5 shadow-sm">
				<div class="flex h-12 w-12 items-center justify-center rounded-lg bg-orange-50 text-orange-500">
					<Dumbbell size={24} />
				</div>
				<div>
			...
			<div class="flex items-center gap-4 rounded-xl border-t-4 border-t-pink-400 bg-white p-5 shadow-sm">
			...
			<div class="flex items-center gap-4 rounded-xl border-t-4 border-t-blue-500 bg-white p-5 shadow-sm">
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
			...
			<div class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm lg:col-span-2">
				<div class="mb-6 flex items-center justify-between">
					...
					<select class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs text-gray-600 outline-none">
			...
			<div class="flex flex-col rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
				...
				<div class="flex flex-col gap-4">
					{#if jadwalTerdekat.length === 0}
						<div class="flex flex-1 items-center justify-center rounded-lg border-2 border-dashed border-gray-200 p-6 text-center text-sm text-gray-500">
					...
					{:else}
						{#each jadwalTerdekat as jadwal (jadwal.id)}
							...
							<div class="flex items-center gap-4">
								<div class="flex h-14 w-12 flex-col items-center justify-center rounded-lg bg-[#117064] text-white shadow-sm">
									<span class="text-lg font-black">{formatTgl.tanggalPendek}</span>
									<span class="text-[10px] font-medium tracking-widest uppercase">{formatTgl.bulanPendek}</span>
								</div>
								<div class="flex-1">
									<h3 class="text-sm font-bold text-gray-800">{jadwal.nama_kegiatan}</h3>
									<p class="text-xs text-gray-500">{formatTgl.hari}, {formatTgl.dmy} • {jadwal.jam_mulai}-{jadwal.jam_selesai}</p>
								</div>
								<span class="rounded-md border border-[#117064] bg-teal-50 px-2 py-1 text-[10px] font-bold text-[#117064]">
				...
				<a href="/dashboard/jadwal" class="mt-6 w-full text-center block cursor-pointer rounded-lg border border-[#117064] py-2.5 text-xs font-bold text-[#117064] transition-colors hover:bg-[#117064] hover:text-white">
					LIHAT SEMUA JADWAL
				</a>
			</div>
		</div>
						...
						{:else}
							{#each balitaTerbaru as balita (balita.id)}
								<tr class="transition-colors hover:bg-gray-50/50">
									...
									<td class="px-6 py-4 text-center font-medium text-[#117064] bg-teal-50/30">{hitungUsia(balita.tanggal_lahir)}</td>
									<td class="px-6 py-4 text-center">
										{#if balita.jenis_kelamin === 'L'}
											<span class="rounded bg-blue-50 text-blue-600 px-2 py-1 text-xs font-bold border border-blue-100">Laki-laki</span>
										{:else}
											<span class="rounded bg-pink-50 text-pink-600 px-2 py-1 text-xs font-bold border border-pink-100">Perempuan</span>
										{/if}
									</td>
									<td class="px-6 py-4 font-medium">{balita.nama_orang_tua}</td>
									<td class="px-6 py-4 text-center">
										<button class="text-[#117064] hover:bg-teal-50 p-2 rounded-lg transition-colors cursor-pointer" title="Lihat Detail">
											<Eye size={18} />
										</button>
									</td>
								</tr>
